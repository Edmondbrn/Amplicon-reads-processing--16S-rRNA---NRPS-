...
tryCatch({
    root = find_root(is_rstudio_project | is_r_package | has_file("analysis_16S_rRNA.Rmd"))
    setwd(root) # set the working directory to the location of this script
}, error = function(e){
    setwd("/home/edmond/scripts_frederik/16S_rRNA")
    cat("Error with the automatic path detection, working directory set to default", getwd())
}) 
```
...
saveRDS(Rarefied_physeq_mean_rounded, file = "output/Rarefied_physeq_mean_rounded.rds")
saveRDS(Rarefied_physeq_median, file = "output/Rarefied_physeq_median.rds")
saveRDS(df_rare_mean, file = "output/df_rare_mean.rds")
```
